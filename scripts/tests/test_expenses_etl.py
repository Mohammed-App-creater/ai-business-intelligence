...
# Load .env at project root so WH_PG_* etc. are available to PGPool.from_env().
# Matches the convention used by scripts/embed_documents.py and FastAPI startup.
# Silent no-op if python-dotenv isn't installed — the user gets a clear
# "Failed to open warehouse pool" message later if env vars are missing.
try:
...
async def main(use_warehouse: bool = False) -> int:
    ...
    def check(label, condition, details=""):
        """details is failure context — only shown when the check fails."""
        nonlocal fail_count
    ...
    feb_mkt = next(
        (r for r in result["category_breakdown"]
         if str(r["period"])[:10] == "2026-02-01" and r["category_id"] == 15),
        None,
    )
    ...
    mar = next(
        (r for r in result["monthly_summary"] if str(r["period"])[:10] == "2026-03-01"),
        None,
    )
